...
@push('styles')
<style>
  .sched-table { width:100%; border-collapse:separate; border-spacing:0; font-size:.85rem; min-width:700px; border-top:1px solid var(--s-line); border-left:1px solid var(--s-line) }
  .sched-table th { padding:12px 8px; font-size:.72rem; font-weight:700; text-transform:uppercase; letter-spacing:.04em; text-align:center; color:var(--s-muted); border-bottom:2px solid var(--s-line); border-right:1px solid var(--s-line); background:var(--s-bg); position:sticky; top:0; z-index:2 }
  .sched-table td { padding:8px; text-align:center; vertical-align:top; border-bottom:1px solid var(--s-line); border-right:1px solid var(--s-line) }
  .sched-time { font-size:.82rem; font-weight:700; color:var(--s-muted); white-space:nowrap; text-align:left !important; width:110px }
  .sched-table th.sched-col-today { background:color-mix(in srgb,var(--s-primary) 10%,var(--s-card)); color:var(--s-primary-dark) }
  .sched-table td.sched-col-today { background:color-mix(in srgb,var(--s-primary) 6%,var(--s-card)) }
...
@endpush
@section('content')
@php $todayKey = ['Monday' => 'senin', 'Tuesday' => 'selasa', 'Wednesday' => 'rabu', 'Thursday' => 'kamis', 'Friday' => 'jumat'][now()->format('l')] ?? null; @endphp
    ...
    <table class="sched-table">
      <thead>
        <tr>
          ...
          @foreach ($days as $day)
          <th class="{{ $day === $todayKey ? 'sched-col-today' : '' }}">{{ ucfirst($day) }}</th>
          @endforeach
        </tr>
      </thead>
      <tbody>
        ...
        <tr>
          ...
          @foreach ($days as $day)
          <td class="{{ $day === $todayKey ? 'sched-col-today' : '' }}">
