...
<section class="portal-hero glass-panel">
  <div>
    <span class="portal-hero-kicker">Dashboard administrator</span>
    <h1>Selamat datang, {{ auth()->user()->full_name ?? auth()->user()->name }}</h1>
    <p>{{ now()->locale('id')->isoFormat('dddd, D MMMM YYYY') }} &mdash; berikut ringkasan data sekolah hari ini.</p>
  </div>
  <div class="portal-hero-side">
    ...
    <div class="portal-hero-chip">
      ...
      <div>
        ...
        <span>{{ $todayAttendance }} catatan kehadiran tercatat</span>
      </div>
    </div>
  </div>
</section>
...
<section class="portal-kpis">
  <article class="portal-kpi">
    <div class="portal-kpi-label"><span>Total Siswa</span><span class="kpi-chip blue"><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M22 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg></span></div>
    <strong class="portal-kpi-value">{{ $totalStudents }}</strong>
    <span class="portal-kpi-note">Siswa aktif</span>
  </article>
  <article class="portal-kpi">
    <div class="portal-kpi-label"><span>Total Guru</span><span class="kpi-chip green"><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg></span></div>
    <strong class="portal-kpi-value">{{ $totalTeachers }}</strong>
    <span class="portal-kpi-note">Guru & wali kelas</span>
  </article>
  <article class="portal-kpi">
    <div class="portal-kpi-label"><span>Total Orang Tua</span><span class="kpi-chip purple"><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg></span></div>
    <strong class="portal-kpi-value">{{ $totalParents }}</strong>
    <span class="portal-kpi-note">Akun orang tua</span>
  </article>
  <article class="portal-kpi">
    <div class="portal-kpi-label"><span>Kelas Aktif</span><span class="kpi-chip rose"><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M2 3h6a4 4 0 0 1 4 4v14a3 3 0 0 0-3-3H2z"/><path d="M22 3h-6a4 4 0 0 0-4 4v14a3 3 0 0 1 3-3h7z"/></svg></span></div>
...
<div class="portal-dashboard-grid">
  <div style="display:grid;gap:20px">
    <section class="portal-panel">
      <div class="portal-panel-header">
        ...
        <a class="btn-gradient" href="{{ route('admin.periods.index') }}">
      ...
      <div style="display:grid;gap:14px">
        <div style="display:flex;align-items:center;gap:14px;padding:16px;border-radius:14px;border:1px solid rgba(255,255,255,.6);background:rgba(255,255,255,.45)">
          <span class="kpi-chip teal" style="width:48px;height:48px">
          ...
          @if ($activePeriod)
            <span style="padding:4px 12px;border-radius:8px;font-size:.78rem;font-weight:700;background:color-mix(in srgb,var(--success) 14%,#fff);color:var(--success)">Aktif</span>
          @else
            <span style="padding:4px 12px;border-radius:8px;font-size:.78rem;font-weight:700;background:color-mix(in srgb,#ef4444 12%,#fff);color:#ef4444">Nonaktif</span>
          @endif
        </div>
        <div style="display:flex;align-items:center;gap:14px;padding:16px;border-radius:14px;border:1px solid rgba(255,255,255,.6);background:rgba(255,255,255,.45)">
          <span class="kpi-chip slate" style="width:48px;height:48px">
    ...
    <section class="portal-panel">
      <div class="portal-panel-header">
        <div>
          ...
          <p>Rekap kehadiran &mdash; klik kartu untuk detail siswa.</p>
        </div>
      </div>
      <div onclick="toggleAttendance(event)" style="display:flex;align-items:center;gap:14px;padding:16px;border-radius:14px;border:1px solid rgba(255,255,255,.6);background:rgba(255,255,255,.45);cursor:pointer;transition:.15s">
        <span class="kpi-chip amber" style="width:48px;height:48px">
          <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>
        </span>
        <div style="flex:1">
          <strong style="display:block">Total Kehadiran</strong>
          <span style="color:var(--muted);font-size:.88rem">{{ $todayAttendance }} catatan hari ini &mdash; klik untuk rincian</span>
        </div>
      ...
      <div id="attendanceDropdown" style="display:none;margin-top:12px">
        ...
        <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:12px">
          @foreach (['present','sick','excused','unexcused'] as $status)
            ...
            @endphp
            <div onclick="showAttendanceModal(this)" data-label="{{ $meta['label'] }}" data-color="{{ $meta['color'] }}" data-students='@json($studentList)' style="cursor:pointer;padding:16px;border-radius:14px;border:1.5px solid rgba(255,255,255,.6);background:rgba(255,255,255,.45);transition:.15s" onmouseover="this.style.borderColor='{{ $meta['color'] }}';this.style.boxShadow='0 2px 8px {{ $meta['color'] }}22'" onmouseout="this.style.borderColor='rgba(255,255,255,.6)';this.style.boxShadow='none'">
              <div style="display:flex;align-items:center;justify-content:space-between">
                <div>
                  <div style="font-size:.75rem;font-weight:600;color:var(--muted);margin-bottom:4px">{{ $meta['label'] }}</div>
                  <div style="font-size:1.4rem;font-weight:800;color:var(--ink)">{{ $items->count() }}</div>
                </div>
                <div style="width:36px;height:36px;border-radius:12px;display:grid;place-items:center;background:color-mix(in srgb,{{ $meta['color'] }} 14%,#fff)">
                  @if ($status === 'present')
  ...
  <section class="portal-panel" style="display:flex;flex-direction:column">
    <div class="portal-panel-header">
      <div>
      ...
      <div class="activity-feed">
        @foreach ($recentAudit as $log)
          <div class="activity-item">
            <span class="activity-icon slate">
    ...
    @endif
    <div style="margin-top:auto;padding-top:14px;border-top:1px solid rgba(255,255,255,.5)">
      <a class="btn-gradient" style="width:100%;justify-content:center" href="{{ route('admin.audit.index') }}">Lihat Semua Aktivitas</a>
    </div>
  </section>
</div>
@endsection
