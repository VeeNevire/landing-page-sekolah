...
@section('content')
@if (!$selectedStudent)
<div class="portal-empty glass-panel" style="border-radius:20px">
...
<section class="portal-hero glass-panel">
  <div>
    <span class="portal-hero-kicker">Ringkasan semester</span>
    <h1>Halo, {{ auth()->user()->full_name ?? auth()->user()->name }}</h1>
    <p>Perkembangan terbaru <strong>{{ $studentReport['name'] }}</strong> pada Semester {{ $studentReport['semester'] }} Tahun Ajaran {{ $studentReport['academic_year'] }}.</p>
  </div>
  <div class="portal-hero-side no-print">
    <div class="portal-hero-chip">
      ...
      <div>
        <strong>{{ $studentReport['name'] }}</strong>
        <span>{{ $studentReport['class'] ?? '' }} &middot; Semester {{ $studentReport['semester'] }}</span>
      </div>
    </div>
    <div style="display:flex;gap:10px;align-items:center">
      ...
      <a class="btn-gradient" href="{{ route('portal.laporan.csv', ['student_id' => $selectedStudentId]) }}">
        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
...
<section class="portal-kpis">
  <article class="portal-kpi">
    <div class="portal-kpi-label"><span>Rata-rata Nilai</span><span class="kpi-chip blue"><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m18 20 4-4-4-4"/><path d="M20 16H9a4 4 0 0 1-4-4V4"/><line x1="2" y1="20" x2="17" y2="20"/></svg></span></div>
    <strong class="portal-kpi-value">{{ number_format($average, 1, ',', '.') }}</strong>
    <span class="portal-kpi-note good">Predikat {{ \App\Helpers\PortalHelper::gradeLetter($average) }} &bull; KKM {{ $studentReport['kkm'] }}</span>
  </article>
  <article class="portal-kpi">
    <div class="portal-kpi-label"><span>Kehadiran</span><span class="kpi-chip green"><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect width="18" height="18" x="3" y="4" rx="2" ry="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg></span></div>
  ...
  <article class="portal-kpi">
    <div class="portal-kpi-label"><span>Ketuntasan Tugas</span><span class="kpi-chip purple"><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg></span></div>
    <strong class="portal-kpi-value">{{ number_format($completion, 0) }}%</strong>
    <span class="portal-kpi-note good">Seluruh tugas telah dinilai</span>
  </article>
  <article class="portal-kpi">
    <div class="portal-kpi-label"><span>Karakter</span><span class="kpi-chip amber"><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg></span></div>
    <strong class="portal-kpi-value">{{ $studentReport['behavior']['discipline'] ?? '-' }}</strong>
    <span class="portal-kpi-note">Disiplin &amp; tanggung jawab</span>
  </article>
</section>
      ...
      <svg class="line-chart" viewBox="0 0 730 270" role="img" aria-label="Grafik tren nilai">
        <defs>
          <linearGradient id="areaFill" x1="0" y1="0" x2="0" y2="1">
            <stop offset="0%" stop-color="#2456e0" stop-opacity=".25" />
            <stop offset="100%" stop-color="#2456e0" stop-opacity="0" />
          </linearGradient>
        </defs>
  ...
  <div style="display:grid;gap:20px">
    <section class="portal-panel">
      ...
      <div class="attendance-grid">
        ...
        <div class="attendance-box"><strong>{{ $studentReport['attendance']['unexcused'] }}</strong><span>Tanpa Ket.</span></div>
      </div>
    </section>
      ...
      <div class="activity-feed">
        @php
          $iconSvg = [
            ...
            'flask' => '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M10 2v7.527a2 2 0 0 1-.211.896L4.72 20.55a1 1 0 0 0 .9 1.45h12.76a1 1 0 0 0 .9-1.45l-5.069-10.127A2 2 0 0 1 14 9.527V2"/><path d="M8.5 2h7"/><line x1="7" y1="12" x2="17" y2="12"/></svg>',
            'trophy' => '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="8" r="6"/><path d="M15.477 12.89 17 22l-5-3-5 3 1.523-9.11"/></svg>',
            'note' => '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M7.9 20A9 9 0 1 0 4 16.1L2 22Z"/></svg>',
            'science' => '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><path d="M12 8v8"/><path d="M8 12h8"/></svg>',
            'book' => '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M4 19.5v-15A2.5 2.5 0 0 1 6.5 2H19a1 1 0 0 1 1 1v18a1 1 0 0 1-1 1H6.5a1 1 0 0 1 0-5H20"/></svg>',
            'mic' => '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect width="14" height="14" x="5" y="1" rx="2.5" ry="2.5"/><path d="M12 20v-6"/><line x1="9" y1="20" x2="15" y2="20"/></svg>',
          ];
          $iconColor = [
            'check' => 'green', 'flask' => 'purple', 'trophy' => 'amber',
            'note' => 'blue', 'science' => 'blue', 'book' => 'slate', 'mic' => 'rose',
          ];
        @endphp
        @foreach ($studentReport['activities'] as $activity)
        <div class="activity-item">
          <span class="activity-icon {{ $iconColor[$activity['icon']] ?? 'slate' }}">{!! $iconSvg[$activity['icon']] ?? '' !!}</span>
