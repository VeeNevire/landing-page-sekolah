...
@php
  $allClassAvg = collect($classList)->pluck('subject_averages')->flatten()->filter();
  $overallAvg = $allClassAvg->count() > 0 ? round($allClassAvg->avg(), 1) : '-';
  $allClassAttendance = collect($classList)->filter(fn($c) => $c['total_attendance_days'] > 0);
...
<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(300px,1fr));gap:18px">
  ...
  @php
    $allAvg = collect($class['subject_averages'])->filter()->values();
    $overallClassAvg = $allAvg->count() > 0 ? round($allAvg->avg(), 1) : null;
  @endphp
  <div class="card card-hover" style="padding:0;overflow:hidden;cursor:pointer" data-class="{{ $class['name'] }}">
    <div style="padding:22px">
      <div style="display:flex;justify-content:space-between;align-items:start;margin-bottom:16px">
        <div>
          <h3 style="margin:0;font-size:1.25rem">{{ $class['name'] }}</h3>
          <span style="color:var(--muted);font-size:.85rem">{{ $class['student_count'] }} siswa aktif</span>
        </div>
        <span style="width:42px;height:42px;border-radius:12px;display:grid;place-items:center;background:color-mix(in srgb,var(--primary-2) 12%,var(--card));color:var(--primary-2);font-weight:900;font-size:.82rem;flex-shrink:0">{{ $class['student_count'] }}</span>
      </div>
      ...
      @if ($class['total_attendance_days'] > 0)
      <div style="display:flex;align-items:center;gap:8px;font-size:.85rem">
        ...
        <span style="font-weight:700;color:{{ $class['attendance_rate'] >= 90 ? 'var(--success)' : ($class['attendance_rate'] >= 75 ? '#b45309' : '#ef4444') }}">{{ $class['attendance_rate'] }}%</span>
    ...
    <div style="border-top:1px solid var(--line);padding:12px 22px;display:flex;justify-content:space-between;align-items:center">
      <a href="{{ route('guru.nilai', ['class' => $class['name']]) }}" class="btn btn-outline" style="min-height:34px;padding:0 16px;font-size:.82rem;border-radius:10px;display:inline-flex;align-items:center;gap:5px" onclick="event.stopPropagation()">
    ...
    <table class="grade-table">
      <thead>
        <tr>
          <th>Kelas</th>
          @php
            $allSubjects = collect($classList)->pluck('subjects')->flatten()->unique('id')->sortBy('name')->values()->all();
          @endphp
      ...
      <tbody>
        @forelse ($classList as $class)
          @php
            $classAvgs = collect($class['subject_averages'])->filter()->values();
            $classAvg = $classAvgs->count() > 0 ? round($classAvgs->avg(), 1) : '-';
          @endphp
          <tr>
            <td><strong>{{ $class['name'] }}</strong></td>
            @foreach ($allSubjects as $subject)
              @php $avg = $class['subject_averages'][$subject['id']] ?? null; @endphp
              <td>
                @if ($avg !== null)
    ...
    <table class="grade-table">
      ...
      <tbody>
        @forelse ($classList as $class)
          ...
          <tr>
            <td><strong>{{ $class['name'] }}</strong></td>
            <td style="color:var(--success);font-weight:700">{{ $class['attendance']['present'] ?? 0 }}</td>
            <td>{{ $class['attendance']['sick'] ?? 0 }}</td>
            <td>{{ $class['attendance']['excused'] ?? 0 }}</td>
            <td>{{ $class['attendance']['late'] ?? 0 }}</td>
            <td style="color:#ef4444;font-weight:700">{{ $class['attendance']['unexcused'] ?? 0 }}</td>
            <td>
              <span style="padding:4px 10px;border-radius:8px;font-size:.78rem;font-weight:700;background:color-mix(in srgb,{{ $class['attendance_rate'] >= 90 ? 'var(--success)' : ($class['attendance_rate'] >= 75 ? '#b45309' : '#ef4444') }} 12%,var(--card));color:{{ $class['attendance_rate'] >= 90 ? 'var(--success)' : ($class['attendance_rate'] >= 75 ? '#b45309' : '#ef4444') }}">{{ $class['attendance_rate'] }}%</span>
            </td>
          </tr>
          @endif
        @empty
